...
On the reversing side of things, being more thorough with the dynamic analysis could have been done to see the decryption in memory being taken place instead of relying on setting breakpoints on a specific API (what if it's resolved and thus is never formally â€œcalledâ€?). Rebasing the addresses with what's in IDA could've also been done to follow program execution flow better.
